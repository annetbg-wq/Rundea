...
const uiCopy: Record<Locale, UiCopy> = {
  ...
  ru: {
    ...
    secret: "Секрет",
    saved: "Сохранённая конфигурация",
...
function App() {
  ...
  useEffect(() => {
    if (!form.serviceName.trim() && serviceNames.length) {
      setForm(current => ({ ...current, serviceName: serviceNames[0]! }));
    }
  ...
  for (const deployment of deployments) {
    if (deployment.status === "READY" && !currentReadyByService.has(deployment.service_name)) {
      currentReadyByService.set(deployment.service_name, deployment);
    }
  }
  ...
  async function saveVariableDrafts() {
    ...
    const changes = variables.filter(v => v.key.trim()).map(v => ({ key: v.key.trim(), value: v.value, secret: v.secret }));
  ...
  async function rollbackDeployment(deployment: Deployment) {
    const confirmed = window.confirm(locale === "ru"
      ? `Откатить ${deployment.service_name} на ${shortSha(deployment.source_commit_sha)}?`
      : `Roll back ${deployment.service_name} to revision ${shortSha(deployment.source_commit_sha)}?`);
    if (!confirmed) return;
    setRuntimeBusyId(deployment.id);
  ...
  async function attachDomain(event: FormEvent) {
    ...
    const response = await fetch(`${api}/v0/domains`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ serviceName: domainForm.serviceName, hostname: domainForm.hostname.trim() }),
    });
    const body = await response.json();
    if (!response.ok) {
  ...
  async function refreshEvents(deploymentId = selectedLogDeploymentId) {
    ...
    const body = await response.json();
    const rows = Array.isArray(body) ? body : Array.isArray(body.events) ? body.events : [];
    setEvents(rows as DeploymentEvent[]);
    setEventsMessage("");
  }
  ...
  function selectService(serviceName: string) {
    ...
    setDomainForm(current => ({ ...current, serviceName }));
  }
  ...
  useEffect(() => {
    window.localStorage.setItem(guideStorageKey, JSON.stringify(guideProgress));
  }, [guideProgress]);
  ...
  function showGuideTarget(target: GuideTarget) {
    ...
    window.setTimeout(() => {
      document.querySelector(`[data-guide="${target}"]`)?.scrollIntoView({ behavior: "smooth", block: "center" });
    }, 80);
  }
  ...
  return <div className="appShell">
    <aside className="sidebar">
      ...
      <div className="workspaceLabel">AICorn Rocket Group <span>⌄</span></div>
      <nav className="primaryNav" aria-label="Primary">
        <button type="button" className={activeSection !== "nodes" ? "active" : ""} onClick={() => navigate("overview")}>
          <span className="navIcon">⌘</span><span>{c.projects}</span><HelpBubble topic="projects" locale={locale}/>
        </button>
        <button type="button" className={activeSection === "nodes" ? "active" : ""} onClick={() => navigate("nodes")}>
          <span className="navIcon">◇</span><span>{c.nodes}</span><HelpBubble topic="nodes" locale={locale}/>
      ...
      <div className="sidebarSection">
        ...
        <button type="button" className="projectItem active" onClick={() => navigate("overview")}>
          ...
          <span><strong>{projectName}</strong><small>{serviceNames.length || 0} services</small></span>
        </button>
      </div>
      <div className="sidebarBottom">
        <button type="button" className="sideUtility" onClick={() => setGuideProgress(current => ({ ...current, active: true, dismissed: false }))}>
          <span>?</span><span>{c.help}</span>{!guideProgress.active && <i>•</i>}
        </button>
        <div className="localeSwitch" aria-label={c.language}>
    ...
    <div className="workspace">
      <header className="topbar">
        ...
        <div className="topActions">
          <span className="liveState"><i></i>{c.live}</span>
          <button type="button" className="guideButton" onClick={() => setGuideProgress(current => ({ ...current, active: !current.active, dismissed: false }))}>✦ {c.guide}</button>
          <button type="button" className="avatar" aria-label="Account">PK</button>
        </div>
      </header>

      <div className="projectHeader">
        <div>
          <span className="eyebrow">{c.project}</span>
          <div className="projectTitleRow"><h1>{projectName}</h1><StatusPill value={currentDeployment ? "READY" : deployments.length ? "BUILDING" : "EMPTY"}/></div>
          <p>GitHub → Rundea Control Plane → your node → Docker</p>
        </div>
        <div className="projectActions">
          <select aria-label={c.service} value={selectedService} onChange={event => selectService(event.target.value)}>
            <option value="">{c.noService}</option>
            {serviceNames.map(name => <option key={name} value={name}>{name}</option>)}
          </select>
          <button type="button" className="primaryAction" onClick={() => { navigate("overview"); setShowComposer(true); }}>+ {c.newDeployment}</button>
        </div>
      </div>

      <nav className="sectionTabs" aria-label="Project sections">
        {([
          ["overview", c.overview],
          ...
          ["settings", c.settings],
        ] as Array<[Section, string]>).map(([section, label]) => <button type="button" key={section} className={activeSection === section ? "active" : ""} onClick={() => navigate(section)}>{label}</button>)}
      </nav>
      ...
      <div className="contentArea">
            ...
            <div className="mainColumn">
              <article className={`panel deployComposer ${guideClass("repository")} ${guideClass("deploy")}`} data-guide="repository">
                <div className="panelHeader">
                  <div><span className="panelEyebrow">SHIP</span><h2>{c.newDeployment}</h2><p>{c.immutableRevisions}. {c.encryptedSnapshots}.</p></div>
                  <div className="panelHeaderActions"><HelpBubble topic="deployments" locale={locale}/><button type="button" className="iconButton" onClick={() => setShowComposer(value => !value)}>{showComposer ? "−" : "+"}</button></div>
                </div>
                {showComposer && <form className="deploymentForm" onSubmit={submit}>
                  <div className="fieldGrid two">
                    <label>{c.service}<input required placeholder="api" value={form.serviceName} onChange={event => setForm({ ...form, serviceName: event.target.value })}/></label>
                    <label>{c.node}<span className="labelWithHelp"><HelpBubble topic="nodes" locale={locale}/></span><select required data-guide="node" className={guideClass("node")} value={form.nodeId} onChange={event => setForm({ ...form, nodeId: event.target.value })}><option value="">Select node</option>{nodes.map(node => <option key={node.id} value={node.id}>{node.name} · {node.status}</option>)}</select></label>
                  </div>
                  ...
                  <div className="fieldGrid three">
                    <label>{c.revision}<input required placeholder={form.sourceDelivery === "BROKER" ? "40-char commit SHA" : "main"} value={form.sourceRef} onChange={event => setForm({ ...form, sourceRef: event.target.value })}/></label>
                    <label>{c.delivery}<span className="labelWithHelp"><HelpBubble topic="sourceBroker" locale={locale}/></span><select value={form.sourceDelivery} onChange={event => setForm({ ...form, sourceDelivery: event.target.value as "DIRECT" | "BROKER" })}><option value="DIRECT">DIRECT</option><option value="BROKER">BROKER · exact SHA</option></select></label>
                    <label>Dockerfile <small>optional</small><input placeholder="Auto-detect" value={form.dockerfile} onChange={event => setForm({ ...form, dockerfile: event.target.value })}/></label>
                  </div>
                  <div className="fieldGrid three">
                    <label>Container port<input required type="number" value={form.containerPort} onChange={event => setForm({ ...form, containerPort: event.target.value })}/></label>
                    <label>Host port<input required type="number" value={form.hostPort} onChange={event => setForm({ ...form, hostPort: event.target.value })}/></label>
                    <label>{c.healthcheck}<span className="labelWithHelp"><HelpBubble topic="healthcheck" locale={locale}/></span><input placeholder="Auto /health" value={form.healthcheckPath} onChange={event => setForm({ ...form, healthcheckPath: event.target.value })}/></label>
                  </div>
                  <div className="deployFooter" data-guide="deploy">
                    <div><strong>{form.sourceDelivery === "BROKER" ? "Source Broker" : "Direct Git source"}</strong><small>{form.sourceDelivery === "BROKER" ? c.sourceBroker : "Exact commit identity is persisted after build."}</small></div>
                    <button type="submit" className="deployButton" disabled={!form.nodeId}>{c.deploy}<span>↗</span></button>
                  </div>
              ...
              <article className="panel recentPanel">
                <div className="panelHeader compact"><div><h2>{c.recentDeployments}</h2><p>{selectedService || projectName}</p></div><button type="button" className="textButton" onClick={() => navigate("deployments")}>{c.allDeployments} →</button></div>
                <DeploymentRows deployments={deploymentList.slice(0, 4)} locale={locale} nodes={nodes} currentReadyByService={currentReadyByService}/>
              </article>
            </div>

            <div className="sideColumn">
              <article className={`panel healthPanel ${guideClass("healthy")}`} data-guide="healthy">
                <div className="panelHeader compact"><div><span className="panelEyebrow">CURRENT</span><h2>{selectedService || c.noService}</h2></div><HelpBubble topic="healthcheck" locale={locale}/></div>
                {currentDeployment ? <div className="currentRevision">
                  <div className="healthOrb"><span></span></div>
                  <div><StatusPill value="READY"/><strong>{shortSha(currentDeployment.source_commit_sha)}</strong><small>{formatDate(currentDeployment.created_at)}</small></div>
                </div> : <div className="emptyState"><span>○</span><strong>No healthy revision</strong><small>READY appears only after the real healthcheck passes.</small></div>}
              </article>

              <article className="panel trustPanel">
                <span className="panelEyebrow">TRUST BOUNDARY</span>
                <h2>Credentials stay off the node</h2>
                <p>{c.sourceBroker}</p>
                <div className="trustPath"><span>GitHub App</span><b>→</b><span>Control Plane</span><b>→</b><span>one-time bundle</span><b>→</b><span>Node</span></div>
              </article>
            </div>
          </section>
        </>}
        ...
        {activeSection === "deployments" && <section className={`panel pagePanel ${guideClass("runtime")}`} data-guide="runtime">
          ...
          <div className="deploymentTable">
            <div className="tableHead"><span>Revision</span><span>Node</span><span>Status</span><span>Created</span><span></span></div>
            {deploymentList.length === 0 ? <div className="emptyState wide"><strong>No deployments yet</strong></div> : deploymentList.map(deployment => {
              ...
              return <div className="tableRow" key={deployment.id}>
                <div className="revisionCell"><span className="commitGlyph">⑂</span><div><strong>{deployment.service_name} <code>{shortSha(deployment.source_commit_sha)}</code></strong><small>{deployment.operation === "ROLLBACK" ? `rollback ← ${deployment.rollback_target_id?.slice(0, 8) ?? "history"}` : deployment.source_ref}{isCurrent ? " · current" : ""}</small></div></div>
                <span>{nodes.find(node => node.id === deployment.node_id)?.name ?? deployment.node_id.slice(0, 8)}</span>
                <StatusPill value={deployment.status}/>
                <time>{formatDate(deployment.created_at)}</time>
                <div className="rowActions">
                  {isCurrent && <><HelpBubble topic="restart" locale={locale}/><button type="button" disabled={runtimeBusyId === deployment.id || nodeBusy} onClick={() => void restartDeployment(deployment)}>{c.restart}</button></>}
                  {rollbackable && <><HelpBubble topic="rollback" locale={locale}/><button type="button" className="warning" disabled={runtimeBusyId === deployment.id || nodeBusy} onClick={() => void rollbackDeployment(deployment)}>{c.rollback}</button></>}
                </div>
              </div>;
            })}
          </div>
        ...
        {activeSection === "variables" && <section className={`panel pagePanel ${guideClass("variables")}`} data-guide="variables">
          ...
          {!selectedService ? <div className="emptyState wide"><strong>Select or name a service first</strong></div> : <>
            <div className="variableHeader"><span>KEY</span><span>VALUE</span><span>TYPE</span><span></span></div>
            {savedVariables.map(variable => <div className="variableSaved" key={variable.key}><code>{variable.key}</code><span>{variable.secret ? "••••••••••••" : variable.value ?? ""}</span><span className="typeBadge">{variable.secret ? "SECRET" : "VARIABLE"}</span><button type="button" aria-label={`Delete ${variable.key}`} onClick={() => void deleteVariable(variable.key)}>×</button></div>)}
            {variables.map((variable, index) => <div className="variableEditor" key={index}>
              <input aria-label="Variable key" placeholder="KEY" value={variable.key} onChange={event => updateVariable(index, { key: event.target.value })}/>
              <input aria-label="Variable value" placeholder={variable.secret ? "secret value" : "value"} type={variable.secret ? "password" : "text"} value={variable.value} onChange={event => updateVariable(index, { value: event.target.value })}/>
              <label className="secretCheck"><input type="checkbox" checked={variable.secret} onChange={event => updateVariable(index, { secret: event.target.checked })}/><span>{c.secret}</span></label>
              <button type="button" onClick={() => setVariables(rows => rows.filter((_, i) => i !== index))}>×</button>
            </div>)}
            <div className="configFooter"><span>{savedVariables.length} saved · {variables.length} pending</span><button type="button" className="primaryAction small" disabled={!variables.some(row => row.key.trim())} onClick={() => void saveVariableDrafts()}>{locale === "ru" ? "Сохранить изменения" : "Save changes"}</button></div>
            {message && <p className="inlineMessage">{message}</p>}
          </>}
        </section>}
        ...
        {activeSection === "domains" && <section className={`panel pagePanel ${guideClass("domain")}`} data-guide="domain">
          ...
          <form className="domainAttach" onSubmit={attachDomain}>
            <select required value={domainForm.serviceName || selectedService} onChange={event => setDomainForm({ ...domainForm, serviceName: event.target.value })}><option value="">READY service</option>{readyServices.map(service => <option key={service} value={service}>{service}</option>)}</select>
            <input required placeholder="api.example.com" value={domainForm.hostname} onChange={event => setDomainForm({ ...domainForm, hostname: event.target.value })}/>
            <button type="submit" className="primaryAction small" disabled={!readyServices.length}>{c.attach}</button>
          </form>
          ...
          <div className="domainTable">
            {serviceDomains.length === 0 ? <div className="emptyState wide"><strong>No domains attached</strong><small>Attach one after the service is READY.</small></div> : serviceDomains.map(domain => <div className="domainRow" key={domain.id}>
              <div><strong>{domain.hostname}</strong><small>{domain.service_name} · {nodes.find(node => node.id === domain.node_id)?.name ?? domain.node_id.slice(0, 8)}</small>{domain.last_error && <em>{domain.last_error}</em>}</div>
              <StatusPill value={domain.status}/>
              <div className="rowActions">{domain.status === "ACTIVE" ? <a href={`https://${domain.hostname}`} target="_blank" rel="noreferrer">{c.openHttps}</a> : domain.status !== "DELETING" ? <button type="button" onClick={() => void reconcileDomain(domain.id)}>{c.retry}</button> : null}<button type="button" className="danger" disabled={domain.status === "DELETING"} onClick={() => void removeDomain(domain.id)}>{c.remove}</button></div>
            </div>)}
          </div>
          {domainMessage && <p className="inlineMessage">{domainMessage}</p>}
        </section>}
        ...
        {activeSection === "observability" && <section className={`observabilityGrid ${guideClass("observability")}`} data-guide="observability">
          <article className="panel logPanel">
            <div className="panelHeader page"><div><span className="panelEyebrow">STREAM</span><h2>{c.logs}</h2><p>{c.deploymentStream}</p></div><HelpBubble topic="observability" locale={locale}/></div>
            <select className="deploymentPicker" value={selectedLogDeploymentId} onChange={event => setSelectedLogDeploymentId(event.target.value)}><option value="">Select deployment</option>{deployments.map(deployment => <option key={deployment.id} value={deployment.id}>{deployment.service_name} · {shortSha(deployment.source_commit_sha)} · {deployment.status}</option>)}</select>
            <div className="terminal" aria-live="polite">
              {events.length === 0 ? <div className="terminalEmpty">{eventsMessage || "Waiting for deployment events…"}</div> : events.map((event, index) => <div className="terminalLine" key={event.id ?? `${event.created_at}-${index}`}><time>{event.created_at ? new Date(event.created_at).toLocaleTimeString() : "--:--:--"}</time><span className={`stream ${event.stream ?? event.kind ?? "system"}`}>{event.stream ?? event.kind ?? "EVENT"}</span><code>{event.status ? `[${event.status}] ` : ""}{event.message ?? ""}</code></div>)}
            </div>
          </article>
          <article className="panel metricsPanel">
            <div className="panelHeader compact"><div><span className="panelEyebrow">TELEMETRY</span><h2>{c.metrics}</h2></div><HelpBubble topic="metrics" locale={locale}/></div>
            <div className="metricPreview"><div><span>CPU</span><strong>—</strong></div><div><span>Memory</span><strong>—</strong></div><div><span>Network</span><strong>—</strong></div></div>
            <div className="comingCard"><span>↗</span><div><strong>{c.comingNext}</strong><p>{getHelpCopy(locale, "metrics").body}</p></div></div>
          </article>
        </section>}

        {activeSection === "nodes" && <section className="nodesLayout">
          <article className={`panel pagePanel ${guideClass("node")}`} data-guide="node">
            <div className="panelHeader page"><div><span className="panelEyebrow">COMPUTE</span><h2>{c.nodes}</h2><p>{getHelpCopy(locale, "nodes").body}</p></div><HelpBubble topic="nodes" locale={locale}/></div>
            <div className="nodeList">{nodes.length === 0 ? <div className="emptyState wide"><strong>No nodes yet</strong></div> : nodes.map(node => <button type="button" key={node.id} className={`nodeRow ${selectedNodeId === node.id ? "selected" : ""}`} onClick={() => { setForm(current => ({ ...current, nodeId: node.id })); setQualificationNodeId(node.id); }}><span className={`nodeDot ${node.status.toLowerCase()}`}></span><div><strong>{node.name}</strong><small>{node.id}</small></div><StatusPill value={node.status}/></button>)}</div>
            <form className="addNodeForm" onSubmit={createNode}><input required placeholder={c.nodeName} value={nodeName} onChange={event => setNodeName(event.target.value)}/><button className="secondaryAction" type="submit">+ {c.createNode}</button></form>
            {nodeBootstrap && <div className="bootstrapBox"><div><span>{c.oneTimeToken}</span><strong>{nodeBootstrap.name}</strong></div><code>{bootstrapCommand(nodeBootstrap)}</code><button type="button" onClick={() => void copyBootstrap()}>{c.copyCommand}</button><small>{locale === "ru" ? "Не сохраняем токен в интерфейсе после перезагрузки." : "Rundea does not persist this plaintext token in the UI after refresh."}</small></div>}
            {nodeMessage && <p className="inlineMessage">{nodeMessage}</p>}
          </article>

          <article className={`panel readinessPanel ${guideClass("readiness")}`} data-guide="readiness">
            <div className="panelHeader compact"><div><span className="panelEyebrow">QUALIFICATION</span><h2>{c.readiness}</h2></div><HelpBubble topic="nodeReadiness" locale={locale}/></div>
            <select value={qualificationNodeId} onChange={event => { setQualificationNodeId(event.target.value); setForm(current => ({ ...current, nodeId: event.target.value })); }}><option value="">Select node</option>{nodes.map(node => <option key={node.id} value={node.id}>{node.name} · {node.status}</option>)}</select>
            <div className="readinessHero"><div><span className={`readinessOrb ${selectedNode?.status === "ONLINE" ? "online" : ""}`}></span><div><strong>{selectedNode?.name ?? "No node selected"}</strong><small>{selectedNode?.status === "ONLINE" ? "Agent connected · commands available" : "Agent must be ONLINE"}</small></div></div><button type="button" disabled={!qualificationNodeId || selectedNode?.status !== "ONLINE" || latestQualification?.status === "RUNNING"} onClick={() => void runQualification()}>{latestQualification?.status === "RUNNING" ? c.testing : c.testEgress}</button></div>
            {latestQualification && <div className="probeGrid">{latestQualification.probes.length ? latestQualification.probes.map(probe => <div className={`probe ${probe.ok ? "pass" : "fail"}`} key={probe.name}><div><strong>{probeLabels[probe.name] ?? probe.name}</strong><small>{probe.host}:{probe.port}</small></div><div><b>{probe.ok ? "PASS" : "FAIL"}</b><small>{probe.latencyMs != null ? `${probe.latencyMs} ms` : probe.error ?? "—"}</small></div></div>) : <div className="probePending">{latestQualification.status === "RUNNING" ? "Agent is checking the required routes…" : latestQualification.failure_reason ?? "No probe result"}</div>}</div>}
            {qualificationMessage && <p className="inlineMessage">{qualificationMessage}</p>}
          </article>
        </section>}

        {activeSection === "settings" && <section className="settingsGrid">
          <article className="panel pagePanel"><div className="panelHeader page"><div><span className="panelEyebrow">PROJECT</span><h2>{c.settings}</h2><p>UI preferences and current deployment defaults.</p></div></div><div className="settingRow"><div><strong>{c.language}</strong><small>Controls interface, help and guided mode copy.</small></div><div className="localeSwitch large"><button type="button" className={locale === "en" ? "active" : ""} onClick={() => setLocale("en")}>English</button><button type="button" className={locale === "ru" ? "active" : ""} onClick={() => setLocale("ru")}>Русский</button></div></div><div className="settingRow"><div><strong>{c.guide}</strong><small>State-driven, non-blocking product walkthrough.</small></div><div className="settingsActions"><button type="button" onClick={() => setGuideProgress(current => ({ ...current, active: true, dismissed: false }))}>{c.resumeGuide}</button><button type="button" onClick={() => setGuideProgress(restartGuide())}>{c.restartGuide}</button></div></div></article>
          <article className="panel capabilityPanel"><span className="panelEyebrow">SOURCE SECURITY</span><h2>Private GitHub source path</h2><ul><li>GitHub App JWT stays in Control Plane</li><li>Installation token scoped to one repository</li><li>Authorization removed before codeload download</li><li>Node receives only a one-time source bundle ticket</li></ul></article>
        </section>}

        {guideProgress.active && <aside className="guideCoach" aria-live="polite">
          <div className="guideTop"><span>✦ {c.guide}</span><small>{guideCompletedCount}/{guideSteps.length}</small></div>
          {guideStep && guideCopy ? <>
            <div className="guideProgress"><span style={{ width: `${Math.round((guideCompletedCount / guideSteps.length) * 100)}%` }}></span></div>
            <strong>{guideCopy.title}</strong>
            <p>{guideCopy.body}</p>
            <div className="guideActions"><button type="button" className="guidePrimary" onClick={() => showGuideTarget(guideStep.target)}>{c.showMe}</button><button type="button" onClick={() => setGuideProgress(current => skipGuideStep(current, guideStep.id))}>{c.skipStep}</button><button type="button" onClick={() => setGuideProgress(current => ({ ...current, active: false, dismissed: true }))}>{c.exitGuide}</button></div>
          </> : <><div className="guideDone">✓</div><strong>{c.done}</strong><p>{locale === "ru" ? "Основной путь пройден. Rundea продолжит показывать контекстные подсказки по запросу." : "The core path is complete. Contextual help remains available anywhere you need it."}</p><div className="guideActions"><button type="button" onClick={() => setGuideProgress(current => ({ ...current, active: false }))}>{c.exitGuide}</button><button type="button" onClick={() => setGuideProgress(restartGuide())}>{c.restartGuide}</button></div></>}
        </aside>}
      </div>
    </div>
  </div>;
