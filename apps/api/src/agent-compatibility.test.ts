...
const compatible = {
  ...
  capabilities: ["runtimeMetrics", "managedIngress", "buildArgs", "buildGuardrails", "resourceGuardrails", "artifactRetention", "safePromotion"],
};
...
test("accepts immutable compatible Agent and sorts capabilities", () => {
  ...
  assert.deepEqual(identity.capabilities, ["artifactRetention", "buildArgs", "buildGuardrails", "managedIngress", "resourceGuardrails", "runtimeMetrics", "safePromotion"]);
});
...
test("rejects malformed or duplicate capabilities", () => {
  assert.throws(
    () => validateAgentHello({ ...compatible, capabilities: ["artifactRetention", "buildArgs", "buildGuardrails", "managedIngress", "resourceGuardrails", "runtimeMetrics", "bad value"] }, "staging"),
    /invalid capability/,
  );
  assert.throws(
    () => validateAgentHello({ ...compatible, capabilities: ["artifactRetention", "buildArgs", "buildGuardrails", "managedIngress", "resourceGuardrails", "runtimeMetrics", "runtimeMetrics"] }, "staging"),
