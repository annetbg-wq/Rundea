...
export class GitHubProjectConnector {
  ...
  async inspect(repositoryFullName: string, selectedBranch?: string): Promise<EnrichedGitHubProjectDiscovery> {
    ...
    const packageDirectories = Object.keys(allFiles).filter((path) => basename(path) === "package.json").map(dirname);
    ...
    const monorepoConfidence: DiscoveryConfidence = services.length > 1 ? "CONFIRMED" : base.discovery.monorepo.confidence;
