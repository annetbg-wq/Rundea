...
func parseDockerBytes(value string) (uint64, error) {
	...
	if match == nil { return 0, fmt.Errorf("invalid Docker byte value %q", value) }
	number, err := strconv.ParseFloat(match[1], 64)
	if err != nil || math.IsNaN(number) || math.IsInf(number, 0) || number < 0 { return 0, fmt.Errorf("invalid Docker byte value %q", value) }
	...
	if !ok { return 0, fmt.Errorf("unsupported Docker byte unit %q", match[2]) }
	bytes := number * multiplier
	if bytes > float64(^uint64(0)) || math.IsInf(bytes, 0) { return 0, fmt.Errorf("Docker byte value overflows uint64") }
	return uint64(math.Round(bytes)), nil
}

func parseDockerPair(value string) (uint64, uint64, error) {
	parts := strings.Split(value, "/")
	if len(parts) != 2 { return 0, 0, fmt.Errorf("invalid Docker metric pair %q", value) }
	left, err := parseDockerBytes(parts[0]); if err != nil { return 0, 0, err }
	right, err := parseDockerBytes(parts[1]); if err != nil { return 0, 0, err }
	return left, right, nil
}

type managedContainer struct { ID, DeploymentID string }
type dockerMetricSample struct {
	CPUPercent float64
	MemoryUsageBytes, MemoryLimitBytes, NetworkRxBytes, NetworkTxBytes uint64
}

func (sample dockerMetricSample) validateTransportRange() error {
	for name, value := range map[string]uint64{"memory usage": sample.MemoryUsageBytes, "memory limit": sample.MemoryLimitBytes, "network receive": sample.NetworkRxBytes, "network transmit": sample.NetworkTxBytes} {
		if value > maxSafeMetricCounter { return fmt.Errorf("%s metric exceeds JSON-safe integer range", name) }
	}
	return nil
}
...
func listManagedContainers(ctx context.Context) ([]managedContainer, error) {
	...
	if err != nil { return nil, fmt.Errorf("docker ps for metrics: %w: %s", err, strings.TrimSpace(string(out))) }
	var containers []managedContainer
	for _, line := range strings.Split(strings.TrimSpace(string(out)), "\n") {
		line = strings.TrimSpace(line); if line == "" { continue }
		fields := strings.Fields(line)
		if len(fields) != 2 || !metricDeploymentIDPattern.MatchString(fields[1]) { continue }
...
func sampleDockerContainer(ctx context.Context, containerID string) (dockerMetricSample, error) {
	...
	if err != nil { return dockerMetricSample{}, fmt.Errorf("docker stats: %w: %s", err, strings.TrimSpace(string(out))) }
	parts := strings.Split(strings.TrimSpace(string(out)), "|")
	if len(parts) != 3 { return dockerMetricSample{}, fmt.Errorf("unexpected docker stats format") }
	cpu, err := parseDockerPercent(parts[0]); if err != nil { return dockerMetricSample{}, err }
	memoryUsage, memoryLimit, err := parseDockerPair(parts[1]); if err != nil { return dockerMetricSample{}, fmt.Errorf("memory stats: %w", err) }
	rx, tx, err := parseDockerPair(parts[2]); if err != nil { return dockerMetricSample{}, fmt.Errorf("network stats: %w", err) }
	sample := dockerMetricSample{CPUPercent: cpu, MemoryUsageBytes: memoryUsage, MemoryLimitBytes: memoryLimit, NetworkRxBytes: rx, NetworkTxBytes: tx}
	if err := sample.validateTransportRange(); err != nil { return dockerMetricSample{}, err }
	return sample, nil
}
...
func sendRuntimeMetric(w *writer, deploymentID string, sample dockerMetricSample, health *runtimeHealthSample) error {
	payload := map[string]any{
		...
		"cpuPercent": sample.CPUPercent, "memoryUsageBytes": sample.MemoryUsageBytes,
		"memoryLimitBytes": sample.MemoryLimitBytes, "networkRxBytes": sample.NetworkRxBytes,
		"networkTxBytes": sample.NetworkTxBytes, "at": time.Now().UTC().Format(time.RFC3339Nano),
	}
	if health != nil {
		payload["runtimeHealth"] = health.State
		payload["restartDelta"] = health.RestartDelta
		payload["uptimeSeconds"] = health.UptimeSeconds
		if health.Error != "" { payload["healthError"] = health.Error }
	}
...
func collectRuntimeMetrics(ctx context.Context, w *writer) error {
	...
	if listErr != nil { return errors.Join(healthErr, listErr) }
	...
	for _, container := range containers {
		...
		if value, ok := healthSamples[container.DeploymentID]; ok { copy := value; health = &copy }
		if err := sendRuntimeMetric(w, container.DeploymentID, sample, health); err != nil { return err }
		sent[container.DeploymentID] = struct{}{}
	}
	for deploymentID, health := range healthSamples {
		if _, ok := sent[deploymentID]; ok { continue }
		sample := dockerMetricSample{MemoryLimitBytes: runtimeMemoryLimitBytes}
		if err := sendRuntimeMetric(w, deploymentID, sample, &health); err != nil { return err }
	}
...
func runMetricsLoop(ctx context.Context, w *writer, interval time.Duration) {
	...
	sample := func() {
		...
		if err == nil { lastError = ""; return }
		if errors.Is(err, context.Canceled) || ctx.Err() != nil { return }
		message := err.Error()
		if message != lastError { log.Printf("runtime metrics/health sampling degraded: %s", message); lastError = message }
	}
	sample()
	for {
		select { case <-ticker.C: sample(); case <-ctx.Done(): return }
	}
}
