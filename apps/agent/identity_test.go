...
func TestCurrentAgentIdentity(t *testing.T) {
	...
	for _, required := range []string{"artifactRetention", "buildArgs", "continuousHealth", "managedIngress", "resourceGuardrails", "runtimeMetrics"} {
...
func TestRequireCapabilityCLI(t *testing.T) {
	...
	if handled, code := handleIdentityCLI([]string{"--require-capability=continuousHealth"}, &bytes.Buffer{}); !handled || code != 0 {
		t.Fatalf("continuous health capability handled=%v code=%d", handled, code)
	}
