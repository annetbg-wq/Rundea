...
try {
  ...
  const node = await request(`/v0/workspaces/${workspace.id}/nodes`, {
    method:"POST",
    body:JSON.stringify({ name:`build-handoff-${suffix}` }),
  });
  ...
  agent = spawn(agentBinary, [], {
    ...
    env:{
      ...
      RUNDEA_WORK_DIR:`/tmp/rundea-build-handoff-${suffix}`,
  ...
  await poll("handoff Agent online", async () => {
    ...
    const body = await request(`/v0/workspaces/${workspace.id}/nodes`);
    const row = body.nodes.find((item) => item.id === node.id);
    return row?.status === "ONLINE" && row?.agentCapabilities?.includes("prebuiltImages")
      ? { done:true, value:row }
      : { last:row ?? "missing" };
  }, 45000);
  ...
  if (autodeploy.autodeploy.nodeId !== node.id) throw new Error("canonical deploy target was not pinned to acceptance node");

  const queued = await request(`/v0/services/${service.id}/builds`, {
    method:"POST",
    body:JSON.stringify({ revisionSha:sourceSha, buildArgs:{ RUNDEA_ACCEPTANCE:"1" }, deployAfterPush:true }),
  });
  ...
  if (!deploymentId) throw new Error("automatic deployment was not created after registry push");
  ...
  const readyDeployment = await poll("automatic prebuilt deployment READY", async () => {
    const deployments = await request(`/v0/services/${service.id}/deployments`);
    const row = deployments.deployments.find((item) => item.id === deploymentId);
    if (!row) return { last:"missing" };
    if (row.status === "FAILED") throw new Error(`automatic deployment failed: ${JSON.stringify(row)}`);
    return row.status === "READY" ? { done:true, value:row } : { last:row.status };
  }, 180000);
  if (readyDeployment.source_ref !== sourceSha) throw new Error("deployment source provenance mismatch");
  if (readyDeployment.artifact_image_ref !== artifactRef) throw new Error("deployment immutable artifact mismatch");
  const route = await fetch(`http://127.0.0.1:${pushed.target_host_port}/`);
  const routeBody = await route.text();
  if (!route.ok || !routeBody.includes("Rundea Build Engine automatic handoff READY")) {
    throw new Error(`automatic deployment route failed: ${route.status} ${routeBody.slice(0,200)}`);
  }
  ...
  console.log(JSON.stringify({
    ...
    verified:[
      ...
      "artifact-auto-queued-as-deployment",
      "production-agent-pulled-without-building",
      "automatic-deployment-reached-ready",
    ],
  }, null, 2));
} finally {
